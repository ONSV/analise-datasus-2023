...
No dia 27 de dezembro de 2024 o Ministério da Saúde disponibilizou a base de dados consolidada do Sistema de Informações de Mortes (SIM) para o ano de 2023. Assim, o presente documento apresenta uma análise dos óbitos no trânsito no Brasil, com foco na evolução desse cenário entre 2022 e 2023. Os cálculos dos resultados apresentados aqui podem ser consultados no repositório do Observatório: [github.com/ONSV/analise-datasus-2023](https://github.com/ONSV/analise-datasus-2023){target="_blank"}

# Cenário brasileiro

A quantidade de óbitos anuais no trânsito brasileiro cresceu novamente, porém com uma quantidade bem reduzida. Em 2023, o Brasil apresentou um número de **34.881** óbitos no trânsito, um **aumento de 987 óbitos** em comparação com os dados de 2022, o que representa uma variação percentual de **2,91%**. Os gráficos a seguir apresentam a quantidade absoluta de óbitos por ano no Brasil e a variação percetual entre os anos.
...
plot_anual_abs <- ggplot(mortes_ano_decada, aes(x = ano_ocorrencia, y = n)) +
  ...
  geom_text(aes(label = n), nudge_y = -1200, size = 5, color = "white")
...
O modal **Ônibus** apresentou uma redução de 15,87% e a classificação **Outros** apresentou uma redução de 13,99%.
...
Considerando as vítimas fatais do sexo masculino, a faixa etária com o maior aumento de mortes entre 2022 e 2023 foi a de acima de 80 anos, com uma variação de **12%**. A faixa etária com a maior redução se deu entre 45 a 49 anos, com uma variação de **-1%**.
